...
- Add to the plan header: `Skill catalog index: <SKILL_DIR>/INDEX.md` — written out as the absolute path, never the placeholder
...
- Always append: "Index of all dotnet skills: <SKILL_DIR>/INDEX.md — if you hit a .NET problem not covered by the files above, find the skill there (index → domain file → SKILL.md path) and Read it." — with <SKILL_DIR> expanded to the absolute path.
...
The index and domain files are generated by `skills/tools/gen-dotnet-catalog.sh` in the claudefiles repo (this skill's directory is `claude/skills/dotnet-router/` inside that repo; `setup.sh` at the repo root wires everything on a new machine). If a listed path fails to Read (repo moved/updated), rerun the generator and retry.
