...
The index and domain files are generated by `skills/tools/gen-dotnet-catalog.sh` in `~/dev/claudefiles` (this skill's directory is `~/dev/claudefiles/claude/skills/dotnet-router/`; `~/dev/claudefiles/setup.sh` wires everything on a new machine). If a listed path fails to Read (repo moved/updated), rerun the generator and retry.
